...
export async function POST(request) {
  try {
    ...
    const { classId, text, imageUrl ,fileType,nickname} = body;
    ...
    const result = await pool.query(
      ...
      [classId, croppedText, imageUrl || null,nickname,fileType || null]
    );
...
export async function DELETE(request) {
  try {
    ...
    return NextResponse.json({ success: true, deletedMessage: result.rows[0] });
  } catch (error) {
